...
type FuncBackend struct {
	...
	WriteToVolumeFn  func(id uint32, data []byte) (WriteResult, error)
...
func (f *FuncBackend) WriteToVolume(id uint32, data []byte) (WriteResult, error) {
	return f.WriteToVolumeFn(id, data)
}
